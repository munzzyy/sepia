...
#   dist/sepia-<version>.apk  developer-signed universal APK: GitHub release,
#                                F-Droid reproducible verification, sideload
#   dist/sepia-<version>.aab  upload-key-signed bundle for Play Console
#
# Never run by CI. The keystore lives outside the repo and its password stays
# in the system keyring (secret-tool lookup service sepia-keystore key
...
KEYSTORE="${SEPIA_KEYSTORE:-$HOME/keys/sepia-upload.jks}"
ALIAS=sepia-upload
...
KSPW=$(secret-tool lookup service sepia-keystore key upload) || {
  echo "no keystore password in the keyring (service sepia-keystore key upload)"; exit 1;
}
export KSPW
...
APK_OUT="dist/sepia-$VERSION.apk"
AAB_OUT="dist/sepia-$VERSION.aab"
...
# releases/latest/download/sepia.apk across versions.
cp "$APK_OUT" dist/sepia.apk

echo "== artifacts =="
sha256sum "$APK_OUT" "$AAB_OUT" dist/sepia.apk
